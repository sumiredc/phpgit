...
final class LsFilesUseCase
{
    ...
    public function actionDefault(): Result
    {
        ...
        $list = array_map(
            fn(IndexEntry $indexEntry) => $indexEntry->trackedPath->value,
            $gitIndex->entries
        );
    ...
    public function actionZero(): Result
    {
        ...
        $line = array_reduce(
            $gitIndex->entries,
            fn(string $carry, IndexEntry $indexEntry) => sprintf("%s%s\0", $carry, $indexEntry->trackedPath->value),
            '',
        );
