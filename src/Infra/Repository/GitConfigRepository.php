...
readonly final class GitConfigRepository implements GitConfigRepositoryInterface
{
    ...
    public function create(): void
    {
        if (!touch(F_GIT_CONFIG)) {
            throw new RuntimeException(sprintf('failed to touch: %s', F_GIT_CONFIG)); // @codeCoverageIgnore
        }
        ...
        try {
            if ($fp === false) {
                throw new RuntimeException(sprintf('failed to fopen: %s', F_GIT_CONFIG));  // @codeCoverageIgnore
            }
            ...
            foreach ($data as $v) {
                if (fwrite($fp, $v) === false) {
                    throw new RuntimeException(sprintf('failed to fwrite: %s', $v)); // @codeCoverageIgnore
                }
            }
        } finally {
            fclose($fp);
    ...
    public function get(): GitConfig
    {
        ...
        if ($config === false) {
            throw new RuntimeException(sprintf('failed to load ini: %s', F_GIT_CONFIG)); // @codeCoverageIgnore
        }
