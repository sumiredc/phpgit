...
describe('parse', function () {
    it('should match to properties', function (GitIndexHeader $header) {
...
describe('addEntry', function () {
    it('should add entry', function (array $entries) {
        $expectedCount = 0;
        $index = GitIndex::new();
    ...
    it('should overwrite entry when add same entry', function (
        IndexEntry $oldEntry,
        IndexEntry $newEntry,
        string $key
    ) {
    ...
    it('should sorted by key entries', function (array $entries, array $order) {
        $expected = 0;
        $index = GitIndex::new();
...
describe('entriesBlob', function () {
    it('should match to entries blob', function (array $entries, string $expected) {
        $index = GitIndex::new();
        foreach ($entries as $entry) {
            $index->addEntry($entry);
        }
...
describe('asBlob', function () {
    it('should match to blob', function (array $entries, string $expected) {
        $index = GitIndex::new();
        foreach ($entries as $entry) {
            $index->addEntry($entry);
        }
...
describe('existsEntry', function () {
    it('should match results to exists entry', function (string $path, bool $expected) {
        $entries = array_map(fn(string $filename) => IndexEntry::new(
            FileStat::new([
                'dev' => 16777232,
                'ino' => 63467197,
...
describe('existsEntryByFilename', function () {
    it('should match results to exists entry by path', function (string $path, bool $expected) {
        $entries = array_map(fn(string $filename) => IndexEntry::new(
            FileStat::new([
                'dev' => 16777232,
                'ino' => 63467197,
...
describe('removeEntryByFilename', function () {
    it('should removes entry', function (string $path) {
        $entries = array_map(fn(string $filename) => IndexEntry::new(
            FileStat::new([
                'dev' => 16777232,
                'ino' => 63467197,
